...
namespace NetCore.Profiler.Extension.Session
{
    public interface IMemoryProfilingSession : IBaseSession
    {
        ...
        List<ManagedMemoryData> ManagedMemoryStatistics { get;}

        List<UnmanagedMemoryData> UnmanagedMemoryStatistics { get;}

        List<DataTypeMemoryStatistics> DataTypeMemoryStatistics { get; }

        List<DataTypeAllocationStatistics> DataTypeAllocationStatistics { get; }

        List<DataTypeMemoryUsage> GetGarbageCollectorSamples(ulong id);
