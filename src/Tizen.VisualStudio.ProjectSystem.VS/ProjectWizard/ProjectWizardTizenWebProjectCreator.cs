...
{
    internal class ProjectWizardTizenWebProjectCreator: IWizard
    {
        ...
        public void RunStarted(object automationObject, Dictionary<string, string> replacementsDictionary, WizardRunKind runKind, object[] customParams)
        {
            ...
            message = executor.RunTzCmnd(string.Format("/c tz new -T web -t \"{0}\" -w \"{1}\" -p \"{2}\"", template, solDir, prjName));
            if (message.Contains("error:"))
            {
            ...
            string projPath = Path.Combine(solDir, prjName, prjName + ".csproj");
