...
{
    internal class ProjectWizardTizenDotNetProjectCreator: IWizard
    {
        ...
        public void RunStarted(object automationObject, Dictionary<string, string> replacementsDictionary, WizardRunKind runKind, object[] customParams)
        {
            ...
            string type =
                replacementsDictionary.TryGetValue("$tizenprojecttype$", out type)
                ? type : "dotnet";
            ...
            if(nWizard.MultiTemplateValue != null)
                message = executor.RunTzCmnd(string.Format("/c tz new -T dotnet -t \"{0}\" -w \"{1}\" -p \"{2}\"  -P \"{3}\"", template, solDir, prjName, nWizard.MultiTemplateValue));
            else
                message = executor.RunTzCmnd(string.Format("/c tz new -T dotnet -t \"{0}\" -w \"{1}\" -p \"{2}\"", template, solDir, prjName));
            ...
            // Create and Add Build file in Dotnet Project
            prjHelperInstance.createBuildFile(solDir, prjName);
            ...
            if (!initProj)
            {
                ...
                try
                {
                    string projectFolder = Path.Combine(solDir, prjName);
                    ...
                    foreach (EnvDTE.Project project in ListOfProjectsInSolution)
                    {
                        ...
                        {
                            var projPath = currentFile.Replace("/", "\\");
                            if (!projPath.Equals(project.FullName))
